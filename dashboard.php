...
<div class="mb-6 md:mb-8">
    <h2 class="text-sm sm:text-lg font-bold text-slate-800 mb-3 sm:mb-4 flex items-center gap-2">
        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-3 sm:p-4">
                <?php if (!empty($member['progetti'])): ?>
                    <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 sm:mb-3">Progetti (<?php echo count($member['progetti']); ?>)</h4>
                    <div class="space-y-2 mb-3 sm:mb-4 max-h-48 sm:max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                           ...
                           style="background-color: <?php echo $coloreProgetto; ?>">
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs sm:text-sm font-medium text-slate-700 group-hover:text-slate-900 truncate"><?php echo e($progetto['titolo']); ?></p>
                                    <p class="text-xs text-slate-500 mt-0.5 truncate"><?php echo e($progetto['cliente'] ?? 'Cliente non specificato'); ?></p>
                                </div>
                                <span class="text-xs font-medium px-2 py-1 rounded flex-shrink-0 <?php echo $isScaduto ? 'bg-red-100 text-red-600' : 'bg-white/70 text-slate-600'; ?>">
                                    <?php echo $scadenza; ?>
                                </span>
                            </div>
                        </a>
                ...
                <?php if (!empty($member['task'])): ?>
                    <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 sm:mb-3">Task in corso (<?php echo count($member['task']); ?>)</h4>
                    <div class="space-y-2 max-h-48 sm:max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                        ...
                        <a href="progetto_dettaglio.php?id=<?php echo $task['progetto_id']; ?>" class="block p-2 rounded-lg hover:bg-slate-50 transition-colors">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs sm:text-sm text-slate-700 truncate"><?php echo e($task['titolo']); ?></p>
                                    <p class="text-xs text-slate-400 truncate"><?php echo e($task['progetto_titolo']); ?></p>
                                </div>
                                <?php if ($scadenzaTask): ?>
                                <span class="text-xs text-slate-400 flex-shrink-0"><?php echo $scadenzaTask; ?></span>
                                <?php endif; ?>
                            </div>
                        </a>
                        <?php endforeach; ?>
